...
score (TDplus) to identify CDK12-mutated tumors [Popova et al., Cancer Res 2016]. 
...
For instance if the data that we want to analyse is based on hg38, one would 
need to download the  
`ChAS 4.2 Analysis Files for OncoScan CNV Array hg38 NA36.r5` zip file from the 
ThermoFisher website (http://www.thermofisher.com), extract the and run the .
...
Note that the 21p arm have been removed from the coverage (`oncoscan.cov <- oncoscan_na33.cov[seqnames(oncoscan_na33.cov) != "21p"]`)since it is not fully 
covered. Any result on this arm would be biased.
## HRD scores
